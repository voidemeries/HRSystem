...
public class GetTrainingSupportRequestsQueryHandler : IRequestHandler<GetTrainingSupportRequestsQuery, List<TrainingSupportRequestDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly ICurrentUserService _currentUserService;

    public GetTrainingSupportRequestsQueryHandler(IApplicationDbContext context, ICurrentUserService currentUserService)
    {
        _context = context;
        _currentUserService = currentUserService;
    }

    public async Task<List<TrainingSupportRequestDto>> Handle(GetTrainingSupportRequestsQuery request, CancellationToken cancellationToken)
    {
        var currentEmployeeId = _currentUserService.EmployeeId
            ?? throw new UnauthorizedAccessException("User not authenticated.");

        var query = _context.TrainingSupportRequests
        ...
        return results.Select(r => new TrainingSupportRequestDto
        {
            Id = r.Id,
